...
CRCCheck force
;TargetMinimalOS 6.0    
BrandingText "Copyright © BloodSpider"
...
SetOverwrite try
  ...
  File "${SOURCEDIR}\BloodSpider.Communication.dll"
  File "${SOURCEDIR}\BloodSpider.Services.Common.dll"
  File "${SOURCEDIR}\BloodSpider.Services.Windows.exe.config"
  File "${SOURCEDIR}\UsbLibrary.dll"
  File "${SOURCEDIR}\..\..\..\BloodSpider.Services.Windows.NotifyIcon\bin\Debug\${PRODUCT_MAIN_EXE}${EXE_EXTENSION}"
...
Section Uninstall
  ...
  Delete "$INSTDIR\BloodSpider.Communication.dll"
  Delete "$INSTDIR\BloodSpider.Services.Common.dll"
  Delete "$INSTDIR\BloodSpider.Services.Windows.exe.config"
  Delete "$INSTDIR\UsbLibrary.dll"
  Delete "$INSTDIR\${PRODUCT_MAIN_EXE}${EXE_EXTENSION}"

  Delete "$INSTDIR\BloodSpider.Services.Windows.InstallLog"
